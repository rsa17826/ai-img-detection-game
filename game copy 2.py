...
def updateFacesList():
  ...
  try:
    ...
    mtcnn = MTCNN(image_size=160, margin=20,min_face_size=40, keep_all=True, device=device, post_process=False)
...
def run_ai_inference(frame_rgb_small, scale_w, scale_h):
    ...
    try:
        with torch.inference_mode():
            # 1. Detection is the heaviest part
            ...
            if boxes is not None:
                ...
                for box, prob in zip(boxes, probs):
                    ...
                    # Slicing the image
                    crop = frame_rgb_small[max(0, int(box[1])):int(box[3]), 
                                           max(0, int(box[0])):int(box[2])]
                    if crop.size > 0:
                        # Resize here (CPU)
                        face_crops.append(cv2.resize(crop, (160, 160)))
                        valid_boxes.append((x1, y1, x2, y2))

                if face_crops:
                    # 2. VECTORIZED BATCHING (Faster than a for-loop of tensors)
                    # Create one big numpy array first
                    batch_np = np.stack(face_crops).astype(np.float32)
                    
                    # Normalize all at once (Vectorized math is much faster)
                    batch_np = (batch_np - 127.5) / 128.0
                    
                    # Upload to GPU in one single transfer
